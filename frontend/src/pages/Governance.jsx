...
function Acknowledgements() {
  ...
  const ack = async () => {
    setError("");
    try {
      await acknowledgePolicy({ policy_id: Number(selectedPolicy), employee_id: currentEmployeeId });
      load();
    } catch (err) { setError(err.message); }
  };
...
function Audits() {
  ...
  const submit = async (e) => {
    ...
    setError("");
    try {
      await createAudit({ ...form, department_id: form.department_id ? Number(form.department_id) : null, date: form.date || null });
